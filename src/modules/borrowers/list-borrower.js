...
const listBorrower = async (query = {}) => {
  // const response=await Borrower.find()
  ...
  const resultsPromise = Borrower.find({ ...filter, ...filters }).sort(sorted);

  const countPromise = Borrower.countDocuments({ ...filter, ...filters });
